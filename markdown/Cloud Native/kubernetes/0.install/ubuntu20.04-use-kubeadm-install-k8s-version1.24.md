# ubuntu20.04通过kubeadm安装1.24版本以上k8s
...
5. 安装依赖
   ...
   ```bash
   sudo apt-get install -y apt-transport-https ca-certificates curl software-properties-common gnupg2 ipvsadm
   ```
...
6. 安装k8s的apt源
   ...
   #加载源，此时会报错，并提示缺少key
   apt update
   #根据报错添加对应的key，讲key值：B53DC80D13EDEF05改成自己的
...
2. 配置containerd
   ...
   #修改containerd的配置，修改/etc/containerd/config.toml文件
   将sandbox_image = "registry.k8s.io/pause:3.6" 改成sandbox_image = "registry.aliyuncs.com/google_containers/pause:3.6"
   将SystemdCgroup = false 改成SystemdCgroup = true 
   ```
...
3. 配置containerd加速
   ...
   ```yaml
   #在/etc/containerd/config.toml中，找到[plugins."io.containerd.grpc.v1.cri".registry.mirrors]，并在其下方加入格式如下的内容；
...
4. 安装k8s所需的组件
   ...
   #选择1.24版本进行安装
   apt-get install kubectl=1.24.1-00 kubeadm=1.24.1-00 kubectl=1.24.1-00 -y
   #对kubectl、kubeadm、kubelet打上暂停升级的标记
...
2. 通过配置文件init k8s
   ...
   ```bash
   #生成初始文件，为什么不用命令？要用文件？因为文件的配置更加准确，同时也能增加初始化需要调整哪些参数的理解
   kubeadm config print init-defaults > init-default.yaml
   
   ```

   ```yaml
   #init-default.yaml文件内容注意修改
   ...
   localAPIEndpoint:
     advertiseAddress: 192.168.31.242       #修改成自己的主机IP
     bindPort: 6443
   nodeRegistration:
     criSocket: unix:///var/run/containerd/containerd.sock     #核对runtime的endpoint是不是跟containerd的匹配的上
     imagePullPolicy: IfNotPresent
     name: ubuntu1      #修改成自己的主机名称
     taints: null
   ---
   apiServer:
   ...
   imageRepository: registry.aliyuncs.com/google_containers  #配置拉取仓库
   kind: ClusterConfiguration
   kubernetesVersion: 1.24.0
   networking:
     dnsDomain: cluster.local
     podSubnet: 10.244.0.0/16      #增加pod子网段
   ...
   #如果开启了IPvs建议开启
   #---             
   #apiVersion: kubeproxy.config.k8s.ip/v1alpha1
   ...
   由于配置了containerd的加速，此时能拉取到calico所需的镜像。需要等3-5分钟左右即可；

   ```bash
   kubectl get nodes #此时会看到节点都ready了
   ```
